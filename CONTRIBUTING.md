...
**Embeddings on a preview run on Bedrock, not OpenAI.** Preview stages deliberately carry no OpenAI key — one shared key across every open PR's preview would burn embedding spend continuously — so the paths that embed the file corpus (data-lake ingestion, knowledge search, spidering) fall back to the keyless Bedrock embedder, which authenticates with the stage's own AWS role. That fallback is resolved at the embedding seam by `resolveEmbeddingWithKeylessFallback`, once the stage's real credentials are known; you don't have to configure anything.
...
If you specifically need to exercise the OpenAI embedding path on a preview, an admin can paste a key into Admin Settings, or you can save your own personal key in user settings. Both are read from the database rather than baked into the deploy, so no redeploy is needed — but the admin-settings path is served through a per-process cache with a 5-minute TTL, so give it up to that long (or wait for a cold start) before concluding it didn't take. The keyless fallback is keyed to the **requested model's own provider**, not to cloud credentials in general: a stage holding only a VoyageAI key still falls back to Bedrock while **Default Embedding Model** names an OpenAI model, because the model that was asked for is still unreachable. So supply the credential for whichever provider that setting names, and ingestion goes back to it. Adding the key as a deploy secret does *not* work: an SST secret arrives as a linked Resource, never as the `process.env` value the embedding path would read.
