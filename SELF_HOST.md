...
- **No backfill.** Only chunks vectorized AFTER both variables are set get indexed into OpenSearch. Chunks from before that point keep working (still retrievable) but stay on the brute-force scan path indefinitely - nothing runs automatically to catch them up. This is a correctness-neutral gap: retrieval always re-derives which files you can currently access from live data, so a chunk that isn't in OpenSearch yet just gets scanned instead of returning nothing. Retrieval knows which files those are rather than discovering it per query - a chunk is recorded as resident only once its OpenSearch write comes back successful, and a file missing any of them is routed to the scan path up front. If you enable OpenSearch on a deployment with an existing corpus and want that corpus to get real kNN retrieval rather than staying scan-only forever, you have to re-vectorize it (re-upload, or re-run the ingestion pipeline for it) - enabling the env vars alone only affects what is vectorized from that point on.
