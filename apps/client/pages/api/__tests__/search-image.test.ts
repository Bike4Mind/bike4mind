 ...
 * SSRF, content-type, size cap, and the no-persistence/no-shared-cache promise - are the whole
 * point of the route. Captures the raw GET handler past the baseApi/asyncHandler plumbing and
 * drives the real request path. The SSRF classifier itself is covered by ssrfProtection.test.ts.
 */
...
describe('GET /api/search-image', () => {
  ...
  it('rejects a public NAME that resolves to a private IP, without naming the address', async () => {
    const fetchMock = vi.fn();
    global.fetch = fetchMock as never;
    const { req, res, logger } = request('https://127.0.0.1.nip.io/a.png');
  ...
  it('rejects image/svg+xml - it would run as a scriptable document on the app origin', async () => {
    ...
    const { req, res } = request('https://cdn.example.com/a.svg');
  ...
  it('aborts an oversized body rather than buffering it', async () => {
    ...
    const { req, res } = request('https://cdn.example.com/huge.png');
  ...
  it('streams a valid image back inline, cached privately and never persisted', async () => {
    ...
    const { req, res } = request('https://cdn.example.com/a.webp');
  ...
  it('surfaces an upstream failure instead of serving an empty body', async () => {
    ...
    const { req, res } = request('https://cdn.example.com/missing.png');
