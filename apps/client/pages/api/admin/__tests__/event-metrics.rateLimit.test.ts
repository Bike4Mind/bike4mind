...
/**
 * The event-metrics rate limit, driven through the real baseApi and the real Mongo-backed limiter:
 * the counter lives in the Cache collection, so a mocked store would prove nothing about whether
 * the increment bounds anything. The route's own gate is an isAdmin check, which says who the
 * caller is and not how often they may ask, so this limiter is the only thing bounding the
 * `recache=true` path that skips the 12h result cache.
 *
 * Stubbed: baseApi's environment (logging, the JWT verifier, analytics, connectDB) and the
 * CounterLog read itself. `rateLimit`, `cacheRepository` and `cacheService` stay real - the
 * api-key chain falls straight through for a keyless request, which is the path under test.
 */
...
const mockRefs = vi.hoisted(() => ({
  ...
  currentUserId: 'event-metrics-user-a',
  // Stands in for the route's CounterLog.find(...).sort().limit().lean() chain.
  findCounterLogs: vi.fn(() => ({ sort: () => ({ limit: () => ({ lean: async () => [] }) }) })),
}));
...
vi.mock('@server/middlewares/logging', () => ({
  logging: (req: Record<string, unknown>, _res: unknown, next: () => void) => {
    // `log` and `debug` matter here: the real cacheService calls them, and a logger without them
    // turns every cached read into the route's own 500 handler.
...
// whole point of the file, and it also backs the route's own result cache. Only the CounterLog
// read is stubbed: filling a window means running the allowed request count for real.
vi.mock('@bike4mind/database', async importOriginal => ({
  ...(await importOriginal<typeof import('@bike4mind/database')>()),
  connectDB: vi.fn().mockResolvedValue(undefined),
  CounterLog: { find: mockRefs.findCounterLogs },
}));
...
const EXHAUSTED_USER = `event-metrics-principal-${Date.now()}`;

// Every request carries a distinct eventFilter so it misses the route's own 12h result cache,
// which is keyed on the filters. Otherwise the second request onwards is served from that cache
// and never reaches CounterLog, which would make the "not executed on the refused request"
// assertion vacuous.
let uniqueFilter = 0;
...
const callAs = async (userId: string) => {
  ...
  const { req, res } = createMocks({
    method: 'GET',
    url: '/api/admin/event-metrics',
    query: { eventFilter: `probe-${++uniqueFilter}` },
  });
...
describe('GET /api/admin/event-metrics - per-principal rate limit', () => {
  /**
  ...
  it('serves the last request inside the window, then 429s that principal only', async () => {
    ...
    mockRefs.findCounterLogs.mockClear();
    ...
    expect(mockRefs.findCounterLogs).toHaveBeenCalledTimes(1);
    ...
    expect(mockRefs.findCounterLogs).not.toHaveBeenCalled();
    ...
    expect(await cacheRepository.findByKey(`rate-limit:${EXHAUSTED_USER}:admin-event-metrics`)).toBeTruthy();
  });
});
