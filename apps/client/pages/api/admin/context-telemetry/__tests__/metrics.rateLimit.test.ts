...
/**
 * The context-telemetry/metrics rate limit, driven through the real baseApi and the real
 * Mongo-backed limiter: the counter lives in the Cache collection, so a mocked store would prove
 * nothing about whether the increment bounds anything. The route's own gate is an isAdmin check,
 * which says who the caller is and not how often they may ask, so this limiter is the only thing
 * bounding the uncached Quest.aggregate.
 *
...
const mockRefs = vi.hoisted(() => ({
  ...
  currentUserId: 'context-telemetry-metrics-user-a',
...
import handler, {
  CONTEXT_TELEMETRY_METRICS_RATE_LIMIT as ROUTE_LIMIT,
} from '@pages/api/admin/context-telemetry/metrics';
...
const EXHAUSTED_USER = `context-telemetry-metrics-principal-${Date.now()}`;
...
const callAs = async (userId: string) => {
  ...
  const { req, res } = createMocks({
    method: 'GET',
    url: '/api/admin/context-telemetry/metrics',
  });
...
describe('GET /api/admin/context-telemetry/metrics - per-principal rate limit', () => {
  /**
  ...
  it('serves the last request inside the window, then 429s that principal only', async () => {
    ...
    mockRefs.aggregateQuests.mockClear();
    ...
    expect(mockRefs.aggregateQuests).toHaveBeenCalledTimes(1);
    ...
    expect(mockRefs.aggregateQuests).not.toHaveBeenCalled();
    ...
    expect(
      await cacheRepository.findByKey(`rate-limit:${EXHAUSTED_USER}:admin-context-telemetry-metrics`)
    ).toBeTruthy();
  });
});
