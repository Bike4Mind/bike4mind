...
const handler = baseApi({ requiredScopes: [ApiKeyScope.ADMIN] })
  .use(
    rateLimit({
      limit: CONTEXT_TELEMETRY_METRICS_RATE_LIMIT,
      windowMs: ONE_MINUTE_MS,
      bucket: 'admin-context-telemetry-metrics',
    })
  )
  .get(
    asyncHandler(async (req, res) => {
      ...
      for (const toolsArray of (agg.tools as
        Array<Array<{ toolName: string; invocationCount: number; failureCount: number }>> | undefined) || []) {
      ...
      for (const subagentsArray of (agg.subagents as
        Array<Array<{ delegationCount: number; timeoutCount: number }>> | undefined) || []) {
