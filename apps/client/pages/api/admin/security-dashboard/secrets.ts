...
const handler = baseApi<Request, Response>()
  .get(async (req: Request, res: Response) => {
    ...
    const snapshot = await securityDashboardSnapshotRepository.findLatestByStageAndScanType(stage, 'secrets');

    if (!snapshot) {
      throw new NotFoundError('No secrets security snapshot found for this stage.');
    }

    const { score, counts } = computeCategoryScoreAndCounts('secrets', snapshot.findings ?? []);
  ...
  .post(async (req: Request, res: Response) => {
    ...
    const latest = await securityDashboardSnapshotRepository.findLatestByStageAndScanType(stage, 'secrets');
    ...
    const securityResources = Resource as typeof Resource & {
      SECOPS_SECRETS_DISPATCH_TOKEN?: { value: string };
      GITHUB_ZAP_REF?: { value: string };
    };

    const githubToken =
      // Prefer SST Resource binding in deployed environments
      securityResources.SECOPS_SECRETS_DISPATCH_TOKEN?.value || process.env.SECOPS_SECRETS_DISPATCH_TOKEN;

    const githubRepo =
      process.env.GITHUB_SECRETS_REPO ||
      process.env.GITHUB_CODE_REPO ||
      process.env.GITHUB_ZAP_REPO ||
      'MillionOnMars/lumina5';
    const workflowId = process.env.GITHUB_SECRETS_WORKFLOW_ID || 'secrets-scan.yml';
    const ref =
    ...
    if (!githubToken) {
      return res.status(500).json({
        error: 'GitHub token for triggering secrets scan workflow is not configured.',
      });
    }
    ...
    try {
      ...
      console.error('Error triggering GitHub secrets scan workflow', {
        error,
        githubRepo,
        workflowId,
        ref,
      ...
      return res.status(500).json({
        error: 'Failed to trigger secrets scan workflow',
      });
    }
  });
