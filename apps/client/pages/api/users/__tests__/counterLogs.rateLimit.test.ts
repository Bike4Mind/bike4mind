...
/**
 * The counterLogs rate limit, driven through the real baseApi and the real Mongo-backed limiter:
 * the counter lives in the Cache collection, so a mocked store would prove nothing about whether
 * the increment bounds anything. `requiredScopes` gates API-key callers only, so this limiter is
 * the only thing bounding the browser/JWT admin path the tests below exercise.
 *
 * Stubbed: baseApi's environment (logging, the JWT verifier, analytics, connectDB) and the
 * aggregate itself. `rateLimit`, `cacheRepository` and the api-key chain stay real - the key
 * middlewares all fall straight through for a keyless request, which is the path under test.
 */
...
const mockRefs = vi.hoisted(() => ({
  ...
  currentUserId: 'counter-logs-user-a',
}));
...
// whole point of the file. The activity aggregate is stubbed: filling a window means running the
// allowed request count for real, and the pipeline has its own tests.
...
// cache. Otherwise the second request onwards is served from that cache and never reaches the
// aggregate, which would make the "not executed on the refused request" assertion vacuous.
let uniqueFilter = 0;
...
const callAs = async (userId: string) => {
  ...
  const { req, res } = createMocks({
    ...
    query: { startDate: '2026-01-01', endDate: '2026-01-07', counterName: `probe-${++uniqueFilter}` },
  });
...
describe('GET /api/users/counterLogs - per-principal rate limit', () => {
  ...
  it('serves the last request inside the window, then 429s that principal only', async () => {
    ...
    // Establishes that an allowed request does reach the aggregate, so the assertion below is
    // about the refusal rather than about the stub never being reachable.
    expect(executeFacetCompatible).toHaveBeenCalledTimes(1);
