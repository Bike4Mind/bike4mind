 ...
 *
 * The semantic-search route is the second consumer this was written for (#2709): its access gate
 * resolves membership through `resolveRetrievalLakeScope`, and the budget scope derived afterwards
 * reads the memo rather than paying for a second lookup. Order matters on that path - deriving the
 * scope BEFORE the gate would resolve membership itself and cost the query this saves.
 */
