 ...
 *
 * These are the only paths that read a customer's notebook without the sharing
 * checks in `findAccessibleById`, so the rules live in one place:
 *
 * - platform admin only;
 ...
 * and the client can't spoof, unlike earlier entries. An audit log poisoned by a
 * caller-supplied IP is worse than no IP.
 */
function trustedClientIp(req: SupportReadRequest): string | undefined {
