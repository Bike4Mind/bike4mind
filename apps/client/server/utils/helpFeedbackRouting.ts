 ...
 * (immutable on the schema), so an edit cannot extend the retention window.
 *
...
/**
 * Updates the text of a report already routed for this help event. The sibling row can be absent
 * even on a report whose `contentStored` is true, once the 90-day TTL has swept it, so this
 * upserts rather than updating in place.
 */
async function reviseRoutedComment({
  ...
  // `expiresAt` is immutable on the schema, so $setOnInsert is the only path that sets it: an
  // existing row keeps its original window and a TTL-swept one starts a fresh 90 days from now,
  // which is correct in both cases - the latter is new text, not resurrected text. Left to throw
  // for the same reason the create path above does: a revision the user cannot see fail is a
