...
export function validateWorkItemDescription(value: unknown): string | undefined {
...
export function parseStatusFilter(value: unknown): WorkItemStatus[] | undefined {
  ...
  const statuses = raw
    .map(entry => validateWorkItemStatus(String(entry).trim()))
...
export function validateWorkItemDependencies(value: unknown): string[] | undefined {
  ...
  const ids = value.map(entry => {
    if (typeof entry !== 'string' || !mongoose.isValidObjectId(entry)) {
