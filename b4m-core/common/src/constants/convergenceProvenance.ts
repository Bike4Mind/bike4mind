 ...
 * graph - the actual read/halt logic lives in `apps/client/server/queueHandlers/convergenceKillSwitch.ts`,
 * which builds on this.
 *
 * Lives in common, not next to that enforcement path, because producers outside apps/client have to
 * stamp too (packages/scripts's bulk straggler re-enqueue) and neither b4m-core nor packages/scripts
 * can import from apps/client - the same boundary that put `isChunkRebuildPending` in `./chunking`.
 *
