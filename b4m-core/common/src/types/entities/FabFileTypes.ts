...
export interface IFabFile {
   ...
   * Absent until a file is chunked; null once chunked if the file yields no extractable text. Nulled
   * by FAB_FILE_CONTENT_REWRITE_PATCH on a byte rewrite, and by the chunk pass itself on a re-chunk
   * that yields no text, so a stale fingerprint never outlives its text.
   */
