...
// signature is only ever valid as the LAST thing in the URL, which is the only shape signImageUrl
// ever produces and the only shape that can't be trivially extended with attacker data past it.
...
/**
 * Appends a signature query param by string concatenation (never reparses or re-serializes the
 * existing query string - see the file-level comment). Returns the URL unchanged if it fails to
 * parse as a URL, or if it already carries a trailing signature.
 */
...
export function verifyImageUrlSignature(signedUrl: string, secret: string): boolean {
  if (KNOWN_PLACEHOLDER_SECRETS.has(secret)) return false;
