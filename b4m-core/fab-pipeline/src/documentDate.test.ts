...
describe('parseFrontmatterDate', () => {
  ...
  it('refuses a key-shaped prose line whose value only STARTS with a date', () => {
    // Every line here passes the shape gate, so it is the whole-scalar timestamp check alone that
    // stops the leading date from becoming the document's vintage.
    expect(parseFrontmatterDate('---\ncreated: 2016-02-02 was the day we shipped\n---\nmore')).toBeNull();
  });

  it('still reads the date when the block carries a YAML comment line', () => {
    const text = '---\n# generated by the export pipeline\ntitle: Report\ndate: 2019-03-04\n---\n';
    expect(iso(parseFrontmatterDate(text))).toBe('2019-03-04T00:00:00.000Z');
  });

  it('still reads the date when keys are quoted', () => {
    expect(iso(parseFrontmatterDate('---\n"title": Report\n"date": 2019-03-04\n---\n'))).toBe(
      '2019-03-04T00:00:00.000Z'
    );
  ...
  it('reads a plain date value that carries a trailing YAML comment', () => {
    expect(iso(parseFrontmatterDate('---\ndate: 2019-03-04 # first draft\n---\n'))).toBe('2019-03-04T00:00:00.000Z');
  });
...
describe('acceptDocumentDate', () => {
  ...
  // The declared type has lied once (the .xls string above); a truthy non-Date must be refused,
  // not handed to getTime().
  it('refuses a truthy candidate that is neither a Date nor a string', () => {
    for (const raw of [1551657600000, { getTime: () => 0 }, [2019, 3, 4]]) {
      expect(acceptDocumentDate(raw as unknown as Date, DocumentDateSource.DOCUMENT_PROPERTIES, now)).toBeUndefined();
    }
  });
