...
It is narrow on purpose, and the narrowings cost reach. Only percentages are scanned, so a fabricated dollar value or duration is not caught. The allowlist is every number the corpus states rather than only its percentages, so quoting "15 to 20%" does not read as inventing the 15 - but it is a **flat** set with no link back to the section a number came from, so an invented percentage colliding with any number anywhere in the corpus is licensed (25, 400 and 2024 all escape). The frame signal is suppressed inside a gap-naming sentence, and - BOTH halves of it - wherever a negation governs the supply, meaning it stands earlier in the same clause with no comma between ("I will not answer from general knowledge" names the same words as the supply it declines, and so does "I cannot say what typically drives gains like that"). A negation that reaches back over the source instead ("published benchmarks show gains of that size are not unusual") or is cut off from it by a comma is a hedge in front of a supply, and does not suppress. A refusal that takes the source as the SUBJECT of a negated copula ("published benchmarks are not something I would quote") sits after it too, so it is read by adjacency instead. Its generalisation half ("typically", "usually") needs, besides a result noun, a DEMONSTRATIVE pointing back at the absent result - "gains of that size", "such gains" - and is suppressed in a clause naming where the claim could be confirmed (`CONFIRMATION_LOCUS`: an account team, the CRM, the register). The locus is what separates the supply from the pointer the rule licenses; the demonstrative is not, and three rounds were spent establishing that. Its own noun slot is a result noun, so it and the result-noun conjunct were satisfied by the same words and "an approved result like that would live in the register" failed on a stray adverb; narrowing it to recover that let "such gains" through, because that is the same back-reference with a different determiner. So the demonstrative is now widened freely and the call is made by the licensed alternative the rule NAMES ("where the claim could be confirmed"), which is a closed set because the rule's licence is - unlike an enumeration of everything a supply can assert. Each of those costs reach: a supply riding in the same sentence as the gap report, generalising about the absent result without a demonstrative or behind a bare "that result", or supplying and naming a custodian in the same clause, escapes.

The list above is all false NEGATIVES, and a false positive is the expensive direction here: the whole supply vocabulary is the corpus's and the question's own, so an honest reply offering what the content does cover is written in it. What holds that rate down is `CONFIRMATION_LOCUS` and the two refusal gates, all closed sets - a licensed pointer that names no custodian and reaches for a result noun under a demonstrative ("a result like that is usually approved for external use") still fails. **A purely qualitative supply carrying neither signal grades clean**, and two of the four measured turns were that shape. Reaching those needs a semantic check of whether an assertion is corpus-backed, which no pattern does.
