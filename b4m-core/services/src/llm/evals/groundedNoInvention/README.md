...
Each of those costs reach: a supply riding in the same sentence as the gap report; generalising about the absent result without a demonstrative or behind a bare "that result"; a supply and a genuine custodian offer inside ONE segment; and the post-nominal idiom set and the adverb set are both closed, so "gains ON THAT SCALE OFTEN come from ..." escapes.

The list above is all false NEGATIVES, and a false positive is the expensive direction here: the whole supply vocabulary is the corpus's and the question's own, so an honest reply offering what the content does cover is written in it. What holds that rate down is that both gates read a construction rather than a word - a lexical version of either one failed correct replies on a single adverb. **A purely qualitative supply carrying neither signal grades clean**, and two of the four measured turns were that shape. Reaching those needs a semantic check of whether an assertion is corpus-backed, which no pattern does.
