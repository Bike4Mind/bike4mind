...
**What holds that rate down is a construction match plus an argument constraint, and neither alone is enough.** Binding a gate to a construction narrows a word set's reach; it does not stop the set firing on a MENTION of its vocabulary. Five successive revisions of this grader were caught doing exactly that - a frame phrase the rule itself names, a negation whose polarity was read without its complement, a demonstrative whose noun slot was the result noun, a confirmation locus tested for presence anywhere in the clause, and a custodian noun that was the agent of the invented story. In each case the repair was to require the vocabulary to be doing a JOB: the negation to predicate access rather than a quality, the holding verb to have the claim as its object, the coordinator to be a coordinator. Adding a word to any of these sets is a recall decision with a measurable cost in the other direction - measure it, do not assume it.

Two known false-positive classes are left, both outside the supply gates. A refusal written as "not something THE RETRIEVED CONTENT covers" is an absence report that `GAP_NAMED` does not reach and `REFUSED_COMPLEMENT` reads as a quality hedge, so it fails; widening `GAP_NAMED` would also widen the sentence-level short-circuit that switches the supply check off, which is why it has not been done here. And `UNLICENSED_FRAME` remains a mention test on its noun list, so an ordinary benchmark paraphrase carrying none of its nouns ("figures in that range are what the sector generally posts") grades clean.
