...
describe('researchTaskService - create', () => {
  ...
  describe('autoGeneratedTag against a gated namespace', () => {
    // The tag ends up on a FabFile once the task downloads a file (createFabFile gates it too),
    // but a task created here would already return 200 and only fail later in the background -
    // this check fails the create request itself instead.
    it('rejects a non-admin whose autoGeneratedTag names a static-registry prefix, before creating the task', async () => {
      const params = {
        title: 'Test Research Task',
        description: 'Test Description',
        type: ResearchTaskType.SCRAPE,
        ...
        researchAgentId: 'test-research-agent-id',
        executionType: ResearchTaskExecutionType.ON_DEMAND,
        autoGeneratedTag: { name: 'opti:report', icon: 'tag', color: '#000000' },
      };

      await expect(create(mockUser, params, adapters)).rejects.toThrow(/only an admin can change this data lake/i);
      expect(mockResearchTaskRepo.create).not.toHaveBeenCalled();
    });

    it('allows an admin whose autoGeneratedTag names a static-registry prefix', async () => {
      const adminUser = { ...mockUser, isAdmin: true } as IUserDocument;
      const params = {
        title: 'Test Research Task',
        description: 'Test Description',
        type: ResearchTaskType.SCRAPE,
      ...
      (mockResearchTaskRepo.create as Mock).mockResolvedValueOnce(mockResearchTask({ userId: adminUser.id }));

      await expect(create(adminUser, params, adapters)).resolves.toBeDefined();
      expect(mockResearchTaskRepo.create).toHaveBeenCalled();
    });

    it('does not reject an autoGeneratedTag that names no gated namespace', async () => {
      const params = {
        title: 'Test Research Task',
        description: 'Test Description',
        type: ResearchTaskType.SCRAPE,
        ...
        researchAgentId: 'test-research-agent-id',
        executionType: ResearchTaskExecutionType.ON_DEMAND,
        autoGeneratedTag: { name: 'quarterly-report', icon: 'tag', color: '#000000' },
      };
      (mockResearchTaskRepo.create as Mock).mockResolvedValueOnce(mockResearchTask({ userId: mockUser.id }));

      await expect(create(mockUser, params, adapters)).resolves.toBeDefined();
    });
  });
});
