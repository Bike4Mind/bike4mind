...
describe('researchTaskService - process', () => {
  ...
  beforeEach(() => {
    ...
    mockFileTagRepo = {
      findByIdAndUserId: vi.fn(),
      create: vi.fn(),
      findByFoldedNameAndUserId: vi.fn(),
    };
    mockLLM = {
      complete: vi.fn(),
      getModelInfo: vi.fn(),
  ...
  it('should create new tag when research task has autoGeneratedTag that does not exist', async () => {
    ...
    // Assert
    // createFileTag IS the find-or-create - it folds case, so resolving an existing document is its
    // job, not this caller's. Pre-checking here for an exact name would report a reuse as "created"
    // and would miss a name the user holds in another casing.
  ...
  it('should reuse existing tag when research task has autoGeneratedTag that already exists', async () => {
    ...
    const { tagService } = await import('..');
    (tagService.createFileTag as Mock).mockResolvedValue(mockExistingTag);

    // Act
    await process(mockUser, { id: taskId }, adapters);

    // Assert
    // The reuse happens inside createFileTag; what this layer owes is applying what came back
    // rather than a freshly minted document.
    expect(tagService.createFileTag).toHaveBeenCalledWith(mockUser.id, autoGeneratedTag, adapters);
    expect(fabFilesService.createFabFile).toHaveBeenCalledWith(
      mockTask.userId,
      expect.objectContaining({
  ...
  it('should apply the research tag to every file it creates', async () => {
    // Arrange
    const taskId = 'test-task-id';
