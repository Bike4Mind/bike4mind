...
const processScrape = async (
  ...
  if (researchTask.fileTagId) {
    ...
    // createFileTag resolves a name the user already holds in any casing to that document, so this
    // no longer pre-checks for an exact one: doing both would just report "created" for a reuse.
    const tag = await tagService.createFileTag(user.id, researchTask.autoGeneratedTag, adapters);
    tagsToApply.push(tag);
    logger?.info(`🏷️ [TAG_RESOLVED] Applying tag: ${tag.name} (${tag.id})`);
  }
...
const processDeepResearch = async (
  ...
  if (researchTask.fileTagId) {
    ...
    // See the sibling above: createFileTag is the find-or-create, folding case.
    const tag = await tagService.createFileTag(user.id, researchTask.autoGeneratedTag, adapters);
    tagsToApply.push(tag);
    logger?.info(`🏷️ [TAG_RESOLVED] Applying tag: ${tag.name} (${tag.id})`);
  }
