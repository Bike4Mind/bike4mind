...
/**
 * The third of the three call sites that resolve a research task's auto-generated tag - the other
 * two are in process.ts. All three delegate to tagService/createFileTag, which folds case: pinning
 * that here is what stops one site drifting back to an exact-name pre-check, which would mint a
 * second document for a name the user already holds in another casing.
 */
describe('prepareTagsForResearchTask', () => {
  const user = { id: 'user-1' } as IUserDocument;
  const autoGeneratedTag = { name: 'Research: Q3', icon: 'tag', color: '#FF5733' };
  const fileTags = { findByIdAndUserId: vi.fn() };
  const adapters = { db: { fileTags } } as unknown as Parameters<typeof prepareTagsForResearchTask>[1];

  const task = (overrides: Partial<IResearchTaskScrape>) =>
    ({ id: 'task-1', userId: user.id, ...overrides }) as IResearchTaskScrape;

  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('delegates an auto-generated tag to createFileTag rather than pre-checking the exact name', async () => {
    const held = { id: 'held-1', name: 'research: q3' };
    (tagService.createFileTag as Mock).mockResolvedValue(held);

    const tags = await prepareTagsForResearchTask({ user, researchTask: task({ autoGeneratedTag }) }, adapters);

    expect(tagService.createFileTag).toHaveBeenCalledWith(user.id, autoGeneratedTag, adapters);
    expect(tags).toEqual([held]);
    expect(fileTags.findByIdAndUserId).not.toHaveBeenCalled();
  });

  it('applies an explicitly chosen tag by id without touching createFileTag', async () => {
    const chosen = { id: 'tag-9', name: 'invoices' };
    fileTags.findByIdAndUserId.mockResolvedValue(chosen);

    const tags = await prepareTagsForResearchTask(
      { user, researchTask: task({ fileTagId: 'tag-9', autoGeneratedTag }) },
      adapters
    );

    expect(tags).toEqual([chosen]);
    expect(tagService.createFileTag).not.toHaveBeenCalled();
  });

  it('applies nothing when the task names no tag', async () => {
    expect(await prepareTagsForResearchTask({ user, researchTask: task({}) }, adapters)).toEqual([]);
    expect(tagService.createFileTag).not.toHaveBeenCalled();
  });
});
