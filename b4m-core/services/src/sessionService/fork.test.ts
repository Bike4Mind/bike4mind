...
describe('forkSession', () => {
  ...
  // would fetch a message from ANY session and use its timestamp to bound the fork's range query.
  ...
  it('throws NotFoundError when the message belongs to a different session', async () => {
    ...
    await expect(
      forkSession('caller-1', { sessionId: 'session-1', messageId: 'other-session-msg' }, { db })
