...
describe('FabFileChunkRepository.updateEmbeddingModel', () => {
  ...
  it('leaves an already-labeled chunk alone and fills only its unlabeled siblings', async () => {
    ...
    const labeled = await FabFileChunk.create(makeChunk({ fabFileId: fid('f1'), text: 'already-embedded' }));
    await FabFileChunk.updateOne({ _id: labeled._id }, { $set: { embeddingModel: 'text-embedding-ada-002' } });
    const unlabeled = await FabFileChunk.create(makeChunk({ fabFileId: fid('f1'), text: 'legacy' }));

    await fabFileChunkRepository.updateEmbeddingModel(fid('f1'), 'text-embedding-3-small');

    const byId = new Map(
      (await FabFileChunk.find({ fabFileId: fid('f1') }).lean()).map(c => [String(c._id), c.embeddingModel])
    );
  ...
  it('treats null and empty-string labels as unlabeled, not as a model to preserve', async () => {
    ...
    const nulled = await FabFileChunk.create(makeChunk({ fabFileId: fid('f1'), text: 'nulled' }));
    await FabFileChunk.updateOne({ _id: nulled._id }, { $set: { embeddingModel: null } });
    const blank = await FabFileChunk.create(makeChunk({ fabFileId: fid('f1'), text: 'blank' }));
