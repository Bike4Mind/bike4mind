...
vi.mock('@bike4mind/database', () => ({ Project: {}, safeDropIndex: vi.fn() }));

import { selectSupersededDuplicates, type LiveProjectRow } from './20260813000000_fix-project-live-unique-name-index';

const row = (over: Partial<LiveProjectRow> & { _id: string }): LiveProjectRow => ({
  userId: 'u1',
  name: 'Roadmap',
...
describe('selectSupersededDuplicates (project)', () => {
  it('returns nothing when every (userId, name) is unique', () => {
    expect(selectSupersededDuplicates([row({ _id: 'a' }), row({ _id: 'b', name: 'Backlog' })])).toEqual([]);
  });
