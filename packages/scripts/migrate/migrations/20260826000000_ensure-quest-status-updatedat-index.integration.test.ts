...
describe('ensure-quest-status-updatedat-index migration (real DB)', () => {
  it('builds the compound index the timeout sweep selects on', async () => {
    ...
    // Key order is what makes this usable for `status` equality + an `updatedAt` range; reversed,
    // the sweep could not seek and would scan.
    expect(idx?.key).toEqual({ status: 1, updatedAt: 1 });
