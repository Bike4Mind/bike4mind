...
describe('split-chunk-stall-markers-off-notes migration (real DB)', () => {
  ...
  it('derives noExtractableTextAt for every wording of the old prefix, freeing notes only for handler-written ones', async () => {
    ...
    // This wording only ever existed as a test fixture, so a row holding it is evidence a human
    // typed it. It derives, but it keeps its text.
    expect(b?.notes).toBe('No extractable text: scanned image');
  });
